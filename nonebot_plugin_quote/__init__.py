...
try:
    ...
    with open(plugin_config.inverted_index_path, "w", encoding="UTF-8") as fc:
        json.dump(
            inverted_index, fc, indent=2, separators=(",", ": "), ensure_ascii=False
        )
...
async def reply_handle(bot, errMsg, raw_message, groupNum, user_id, listener):
    ...
    if not image_found:
        await bot.send_msg(
            group_id=int(groupNum), message=MessageSegment.at(user_id) + errMsg
        )
        await listener.finish()
...
save_img = on_regex(
    pattern="^{}上传$".format(re.escape(plugin_config.quote_startcmd)), **need_at
)
...
>>>>>>> upstream
    ...
    if "group" in session_id:
        ...
        inverted_index, forward_index = offer(
            groupNum, image_path, ocr_content, inverted_index, forward_index
        )
        ...
        with open(plugin_config.record_path, "w", encoding="UTF-8") as f:
            json.dump(
                record_dict, f, indent=2, separators=(",", ": "), ensure_ascii=False
            )
...
record_pool = on_startswith(
    "{}语录".format(plugin_config.quote_startcmd), priority=2, block=True, **need_at
)
...
async def record_pool_handle(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    if "group" in session_id:
        ...
        search_info = search_info.replace(
            "{}语录".format(plugin_config.quote_startcmd), ""
        ).replace(" ", "")
...
async def record_help_handle(bot: Bot, event: Event, state: T_State):
    ...
    if "group" in session_id:
        ...
        await bot.call_api(
            "send_group_msg",
            **{"group_id": int(groupNum), "message": MessageSegment.at(user_id) + msg},
        )
...
async def delete_record_handle(bot: Bot, event: Event, state: T_State):
    ...
    if user_id not in plugin_config.global_superuser:
        if (
            groupNum not in plugin_config.quote_superuser
            or user_id not in plugin_config.quote_superuser[groupNum]
        ):
            await bot.call_api(
                "send_group_msg",
                **{
    ...
    imgs = await reply_handle(
        bot, errMsg, raw_message, groupNum, user_id, delete_record
    )

    # 搜索
    is_Delete, record_dict, inverted_index, forward_index = delete(
        imgs, groupNum, record_dict, inverted_index, forward_index
    )
...
alltag = on_command(
    "{}alltag".format(plugin_config.quote_startcmd),
    aliases={
        "{}标签".format(plugin_config.quote_startcmd),
        "{}tag".format(plugin_config.quote_startcmd),
    },
    **need_at,
)
...
async def addtag_handle(bot: Bot, event: Event, state: T_State):
    ...
    flag, forward_index, inverted_index = addTag(
        tags, imgs, groupNum, forward_index, inverted_index
    )
    with open(plugin_config.inverted_index_path, "w", encoding="UTF-8") as fc:
        json.dump(
            inverted_index, fc, indent=2, separators=(",", ": "), ensure_ascii=False
...
async def deltag_handle(bot: Bot, event: Event, state: T_State):
    ...
    tags = (
        str(event.get_message())
        .replace("{}deltag".format(plugin_config.quote_startcmd), "")
        .strip()
        .split(" ")
    )
    ...
    flag, forward_index, inverted_index = delTag(
        tags, imgs, groupNum, forward_index, inverted_index
    )
    with open(plugin_config.inverted_index_path, "w", encoding="UTF-8") as fc:
        json.dump(
...
make_record = on_regex(
    pattern="^{}记录$".format(re.escape(plugin_config.quote_startcmd))
)
    ...
    if raw_message:
        ...
        if data:
            ...
            image_path = os.path.abspath(
                os.path.join(quote_path, os.path.basename(image_name))
            )
            ...
            if "group" in session_id:
                ...
                with open(
                    plugin_config.inverted_index_path, "w", encoding="UTF-8"
                ) as fc:
                    json.dump(
                        inverted_index,
                        fc,
...
render_quote = on_regex(
    pattern="^{}生成$".format(re.escape(plugin_config.quote_startcmd))
)
...
async def render_quote_handle(bot: Bot, event: MessageEvent, state: T_State):
    ...
    if raw_message:
        ...
        if data:
            ...
            response = await bot.call_api(
                "send_group_msg",
                **{"group_id": int(session_id.split("_")[1]), "message": msg},
            )
    else:
...
tags=aaa bbb ccc"""
    ...
    for imgid, img in image_files:
        ...
        if idx % 5 == 0:
            ...
            await bot.send_msg(
                group_id=int(groupNum), message="当前进度{}/{}".format(idx, total_len)
            )
